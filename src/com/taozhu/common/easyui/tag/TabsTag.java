...
import com.taozhu.common.easyui.compont.Tab;
...
public class TabsTag extends TagSupport {
	private static final long serialVersionUID = -8494446149797060968L;
	...
	public StringBuffer end() {
		...
		if (iframe) {
			...
			sb.append("{");
			sb.append("var s = \'<iframe id=\"\'+id+\'\" scrolling=\"no\" frameborder=\"0\"  src=\"about:hhwl\" width=\""+getString(width, "100%")+"\" height=\""+getString(heigth, "99.5%")+"\"></iframe>\';");
			sb.append("return s;");
			sb.append("}");
			sb.append("});");
			sb.append("</script>");
		...
		if (tabs) {
			...
			sb.append("<div id=\"" + id + "\" tabPosition=\"" + tabPosition + "\" border=flase style=\"margin:0px;padding:0px;overflow:hidden;width:"+getString(width, "auto")+";\" class=\"easyui-tabs\" fit=\""+fit+"\">");
			if (!iframe) {
				for (Tab tab : tabList) {
					if (tab.getHref() != null) {
						...
						sb.append("<iframe id=\"\'"+tab.getId()+"\'\" scrolling=\"no\" frameborder=\"0\"  src=\""+tab.getIframe()+"\" width=\""+getString(tab.getWidth(), "100%")+"\" height=\""+getString(tab.getHeigth(), "99.5%")+"\"></iframe>\';");
						sb.append("</div>");
					}
